  ...
  <form  @submit.prevent="submit()">
      ...
      <transition name="fade">
          <div v-if="success && message" class="text-success">
              ...
              <div>
                  <span>A password reset link has been sent to {{email}}</span>
                  if you do not receive an email shortly, feel free to <a href="https://yellowimages.com/contact-us">contact us</a> and we’ll be happy to help
              </div>
          </div>
      </transition>
      <p class="reset">Forgot your password? Please enter your emails address below. You will receive an emails containing instructions to reset your password.</p>
    <div class="form-group pre">
        <input type="text"  placeholder="Email"  :class="{ errors: error }" v-model="email"/>
    </div>
      <div class="error-message">
      <span v-if="error || errors" class="error">{{error}} {{errors}}</span>
      </div>
      <button type="submit" :disabled='isDisabled' @onkeydown="validateEmail(email)" class="btn"><span v-if="this.loaded">Send recover</span>
